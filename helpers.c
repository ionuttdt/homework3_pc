...
char *format(char *username, char *username_, char *password, char *password_) {
	char *form_data = calloc(strlen(username), sizeof(char));
	memcpy(form_data, username, strlen(username));
    form_data = concat(form_data, "=");
    form_data = concat(form_data, username_);
    form_data = concat(form_data, "&");
    form_data = concat(form_data, password);
    form_data = concat(form_data, "=");
    form_data = concat(form_data, password_);
    return form_data;
}
